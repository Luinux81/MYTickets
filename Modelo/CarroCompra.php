...
class CarroCompra{
     ...
     * @param int $cantidad Cantidad de entradas
     */
    public static function editarItem($eid,$tp,$cantidad){
        if($_SESSION['carro']!=""){
            $carro=json_decode($_SESSION['carro'],true);
            for($i=0;$i<count($carro);$i++){
                if($carro[$i]['evento']==$eid && $carro[$i]['tipoentrada']==$tp){
    ...
    /**
     * 
     * Esta funcion recorre el array de entrada e inserta en el array de salida solo los elementos con cantidad mayor que 0
     * 
     * @param array $array
     * @return array
     */
